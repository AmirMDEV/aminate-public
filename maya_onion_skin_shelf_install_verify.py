...
try:
    ...
    button_name = maya_onion_skin.install_maya_onion_skin_shelf_button()
    ...
    for index, child_name in enumerate(child_names):
        ...
        if (
            visible_label == maya_onion_skin.DEFAULT_SHELF_NAME
            or child_name == maya_onion_skin.DEFAULT_SHELF_NAME
    ...
    for child in buttons:
        ...
        try:
            ...
            doc_tag = ""
        if doc_tag != maya_onion_skin.SHELF_BUTTON_DOC_TAG:
            continue
...
""".format(repo_path=str(REPO_ROOT).replace("\\", "\\\\"))
    ...
    if result.get("selected_tab") != "Amir_s_Scripts":
        raise AssertionError(json.dumps(payload, indent=2))
    if result.get("matching_label") != "Onion Skin":
        raise AssertionError(json.dumps(payload, indent=2))
    ...
    print("MAYA_ONION_SKIN_SHELF_INSTALL_VERIFY: PASS")
    return 0
