...
- The simple Dynamic Parenting flow is: pick the moving object, leave `Stay In Current Position At Start?` on if you want it to stay where it is when you first add it, click `Add Object`, click `Pick Parent`, then either click `Snap To Parent` or move the object by hand and use `Maintain Current Offset` before switching.
...
- Dynamic Parenting now includes a built-in gun-and-magazine example right in the tab, plus plainer step-by-step help for what each button does.
- Dynamic Parenting keeps a visible `History` list in the `More` section, and you can jump to a switch, `Delete Picked Switch`, or clear all saved switches for that object.
- `Remove Object` now warns before it runs and removes this tool's constraints, saved parent choices, and saved switches for that object so it can be added again cleanly.
