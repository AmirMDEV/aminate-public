...
def run():
    ...
    success, message = parenting_controller.set_active_setup(setup_lookup[_long_name(driven_b)]["setup_group"])
    _assert(success, message)
    cmds.currentTime(8, edit=True)
    before_world_add = _translation(driven_b)
    cmds.select([driven_b, gun_target], replace=True)
    success, message = parenting_controller.add_targets_from_nodes([gun_target])
    _assert(success, message)
    after_world_add = _translation(driven_b)
    _assert(all(abs(before_world_add[index] - after_world_add[index]) < 0.01 for index in range(3)), "Adding a new parent choice should keep the driven control where it was placed")
    driven_b_setup = parenting_controller.current_setup()
    gun_target_entry_b = next(target for target in driven_b_setup["targets"] if target["driver"] == _long_name(gun_target))
    success, message = parenting_controller.parent_fully_to_target(gun_target_entry_b["id"])
    _assert(success, message)
    after_world_switch = _translation(driven_b)
    _assert(all(abs(before_world_add[index] - after_world_switch[index]) < 0.01 for index in range(3)), "Switching to the new parent should keep the driven control where it was placed")
    cmds.currentTime(8, edit=True)
